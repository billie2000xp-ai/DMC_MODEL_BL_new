...
//    }
    ...
    rcmd_bp_byrp = false;
    is_mat_distref.resize(NUM_BANKS);
    for (size_t i = 0; i < NUM_BANKS; i ++) {
        is_mat_distref[i] = false;
    }
    fg_hit_count.resize(NUM_BANKS);
    for (size_t i = 0; i < NUM_BANKS; i ++) {
        fg_hit_count[i] = 0;
    }
    ecc_wb_cnt = 0;
//    DEBUG(now()<<" after ini, sc_num="<<sc_num);
    next_ps_cycle = ceil(float(tPS)/tDFI); // 第一次巡检的时间点
    current_ps_addr = 0;
    ps_task_counter = 0;
    ps_rank = 0;
    ps_sid = 0;
...
void MemoryController::receiveFromBus(unsigned long long task, bool mask_wcmd, bool ps) {
    ...
    if (DEBUG_BUS) {
        PRINTN(setw(10)<<now()<<" -- R DDR :: Receiving From Data Bus, task="<<task<<", ps="<<ps<<endl);
    }
}
...
unsigned MemoryController::get_rdata_chunk_beats(const TRANS_MSG &msg) const {
    if (!RDATA_MERGE_EN) return 1;

    unsigned total_beats = msg.burst_length + 1;
    unsigned chunk_beats = std::max(1u, MERGE_DATA_SIZE / 32);
    if (chunk_beats > total_beats) {
        chunk_beats = total_beats;
    }
    return chunk_beats;
}
                ...
                if (state.rank == rank) { // same rank
                    if (state_channel == sub_channel) { // same rank, same subchannel
                        if (state.sid == sid) { // same rank, same sid
                            ...
                            state.state->nextWriteAp = max(now() + CalcTccd(true, bus_packet.bl, tCCD_L) +
                                    CalcCmdCycle(rw_cycle, rw_cycle), state.state->nextWriteAp);
                            state.state->nextWriteMask = max(now() + CalcTccd(false, bus_packet.bl, tCCD_L) +
                                    CalcCmdCycle(rw_cycle, rw_cycle), state.state->nextWriteMask);
                        }
            ...
            for (auto &state : bankStates) {
                if (state.rank == rank) { // same rank
                    if (state.sid == sid) { // same rank, same sid
                        ...
                        state.state->nextWrite = max(now() + CalcTccd(false, bus_packet.bl, tCCD_L) +
                                CalcCmdCycle(rw_cycle, rw_cycle), state.state->nextWrite);
...
//                        DEBUG(now()<<" fresh timing, bank="<<bankIndex<<" fst_bankIndex="<<bus_packet.fst_bankIndex<<" lst_bankIndex="<<bus_packet.lst_bankIndex);
                ...
                unsigned ps_addtional_latency = 0;
                if (need_wb) {
                    ps_addtional_latency = 0 + 0;
                } else {
                    ps_addtional_latency = 0;
                }
                ...
                for (size_t i = 0; i < pbr_bank_num; i++) {
                    for (size_t j = 0; j < pbr_bg_num; j++) {
                        ...
                        if (bankIndex == (bus_packet.bankIndex + j * pbr_bank_num)) {
                            ...
                            bankStates[bankIndex].state->stateChangeCountdown = trfcpb + CalcCmdCycle(cmd_cycle, 1);
                            if ((DMC_V580 && IS_LP5) || IS_LP6 || IS_GD2) {
                                bankStates[bankIndex].state->nextActivate1 = max(
                                    now() + trfcpb - unsigned(ceil(OFREQ_RATIO * 2)) + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                    bankStates[bankIndex].state->nextActivate1);
                            } else {
                                bankStates[bankIndex].state->nextActivate1 =
                                    max(now() + trfcpb - unsigned(ceil(OFREQ_RATIO)) + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate1);
                            }
                            bankStates[bankIndex].state->nextActivate2 =
                                max(now() + trfcpb + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                    bankStates[bankIndex].state->nextActivate2);
                            bankStates[bankIndex].state->nextPerBankRefresh =
                                max(now() + trfcpb + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                    bankStates[bankIndex].state->nextPerBankRefresh);
                            bankStates[bankIndex].state->nextAllBankRefresh =
                                max(now() + trfcpb + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                    bankStates[bankIndex].state->nextAllBankRefresh);
                        } else {
                            bankStates[bankIndex].state->nextPerBankRefresh =
                                max(now() + tPBR2PBR + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                    bankStates[bankIndex].state->nextPerBankRefresh);
                            ...
                            if (IS_LP5 || IS_LP6 || IS_GD2) {
                                if (DMC_V580 || IS_LP6) {
                                    bankStates[bankIndex].state->nextActivate1 =
                                        max(now() + tPBR2ACT - unsigned(ceil(OFREQ_RATIO * 2)) +
                                                CalcCmdCycle(cmd_cycle, cmd_cycle),
                                            bankStates[bankIndex].state->nextActivate1);
                                    bankStates[bankIndex].state->nextActivate2 =
                                        max(now() + tPBR2ACT + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                            bankStates[bankIndex].state->nextActivate2);
                                } else {
                                    bankStates[bankIndex].state->nextActivate1 = max(
                                        now() + tPBR2ACT - unsigned(ceil(OFREQ_RATIO)) + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate1);
                                }
                            } else if (IS_LP4) {
                                bankStates[bankIndex].state->nextActivate1 =
                                    max(now() + tRRD_S - unsigned(ceil(OFREQ_RATIO)) + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate1);
                                bankStates[bankIndex].state->nextActivate2 =
                                    max(now() + tRRD_S + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate2);
                            } else if (IS_LP4 || IS_HBM2E || IS_HBM3) {
                                bankStates[bankIndex].state->nextActivate1 =
                                    max(now() + tPBR2ACT - unsigned(ceil(OFREQ_RATIO)) + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate1);
                                bankStates[bankIndex].state->nextActivate2 =
                                    max(now() + tPBR2ACT + CalcCmdCycle(cmd_cycle, cmd_cycle),
                                        bankStates[bankIndex].state->nextActivate2);
        ...
        case READ_CMD : {
            if (RDATA_TYPE == 0) {
                for (auto &trans : transactionQueue) {
                    ...
                    if ((!IECC_ENABLE || !tasks_info[c->task].rd_ecc) && !c->ps && !c->wb){
                        gen_rresp(c->task);
                    }
                    break;
                }
...
void MemoryController::SetRwSyncHint(bool valid, uint8_t target_group, bool is_rw_gray) {
    if (DEBUG_BUS) {
        PRINTN(setw(10) << now() << " -- SYNC_HINT :: ch=" << channel 
               << " valid=" << valid 
               << " block=" << is_rw_gray 
               << " target=" << (unsigned)target_group 
               << (target_group == READ_GROUP ? "(READ)" : 
                  (target_group == WRITE_GROUP ? "(WRITE)" : "(NO_GROUP)")) 
               << endl);
    }

    rw_sync_hint_valid = valid;
    rw_sync_hint_group = target_group;
    rw_sync_gray = is_rw_gray;

    if (valid && target_group != NO_GROUP && target_group != rw_group_state.back()) {
        rw_group_state.push_back(target_group);
        serial_cmd_cnt = 0x0;
        rwgrp_ch_cmd_cnt = 0x0;
    }
}
...
//        unsigned bank_pair_start = sub_channel * pbr_bank_num;
        ...
        if (t->transactionType == DATA_READ) {
            ...
            r_qos_cnt[t->qos] --;
            if (RDATA_TYPE == 1 && t->mask_wcmd==false && !t->ps) {
                if ((!IECC_ENABLE || !tasks_info[t->task].rd_ecc) && !t->ps && !t->wb) {
                    gen_rresp(t->task);
                }
            }
...
//                if (tmp->task == t->task) continue;
            ...
            if ((!IECC_ENABLE || !tasks_info[t->task].wr_ecc) && !t->wb) {
                gen_wresp(t->task);
            }
            que_write_cnt --;
            w_qos_cnt[t->qos] --;
        ...
        if (TIME_ASSERT_EN) {
            if ((now() - trans->timeAdded) > 1000000) {
                ERROR(setw(10)<<now()<<" -- task="<<trans->task<<" address="<<hex<<trans->address<<dec
                        <<" rank="<<trans->rank<<" sid="<<trans->sid<<" bankgroup="<<trans->group<<" bank="<<trans->bankIndex
                        <<" row="<<trans->row<<" matgrp="<<(trans->row&(NUM_MATGRPS-1))<<" ps="<<trans->ps);
...
void MemoryController::update_patrol_scrubbing() {
    ...
    if (ps_rd_queue.size() < 16 && now() >= next_ps_cycle) {
        ...
        trans->issue_size = 0;
        trans->trans_size = trans->data_size; 
        
        // --- B. 动态行大小计算 ---
        unsigned col_size_bytes = NUM_COLS * JEDEC_DATA_BUS_BITS / 8; 
        ...
        // --- E. 挂起到内部指针，等待仲裁 ---
        ps_rd_queue.push_back(trans);

        if (DEBUG_BUS) {
            PRINTN(setw(10) << now() << " -- PS_CMD :: Generated Patrol Read at addr=0x" 
                   << hex << current_ps_addr << dec 
                   << " task=" << trans->task
...
void MemoryController::update_ecc_wb_delay() {
    ...
    while (!delayed_ecc_wb_queue.empty()) {
        ...
        if (now() >= target_time) {
            ...
            if (DEBUG_BUS) {
                PRINTN(setw(10)<<now()<<" -- ECC WB: Delay over! Command pushed to Queue. task=" << front_item.first->task<<endl);
            }
...
void MemoryController::lc(Transaction *t) {
    ...
    if (rw_group_state[0] != NO_GROUP && !t->timeout && t->issue_size == 0 && t->nextCmd != PRECHARGE_PB_CMD
            && !t->act_executing) {
        if (DMC_RW_SYNC_EN) {
            bool is_new_dir_cmd = (rw_group_state[0] == READ_GROUP && t->transactionType == DATA_READ) ||
                                  (rw_group_state[0] == WRITE_GROUP && t->transactionType != DATA_READ);
            bool is_cas_cmd = (t->nextCmd >= WRITE_CMD && t->nextCmd <= READ_P_CMD);
            
            if (rw_sync_gray) {
                if (is_new_dir_cmd && is_cas_cmd) {
                    if (DEBUG_BUS) {
                        PRINTN(setw(10)<<now()<<" -- LC :: SYNC Gray Phase backpress new dir CAS. task="<<t->task<<endl);
                    }
                    return; // 拦截�?CAS，强行拉�?in_write_group 的翻转时�?
                }
            } else {
                if (!is_new_dir_cmd) {
                    if (DEBUG_BUS) {
                        PRINTN(setw(10)<<now()<<" -- LC :: SYNC Stable Phase backpress old dir cmd. task="<<t->task<<endl);
                    }
                    return;
                }
            }
...
bool MemoryController::addTransaction(Transaction *trans) {
    ...
    if (!full()) {
        ...
        if (trans->wb && trans->transactionType != DATA_READ) {
            trans->data_ready_cnt = trans->burst_length + 1;
        }
...
//        }
        ...
        if (((trans->transactionType==DATA_WRITE && !trans->mask_wcmd) || (trans->transactionType==DATA_READ && trans->mask_wcmd)) && RMW_CMD_MODE==0 && IS_LP6 && !trans->wb) {
            gen_cresp(trans->task);
        }
        
        //lp5: full write and mask write
        if ((trans->transactionType==DATA_WRITE) && IS_LP5 && !trans->wb) {
            gen_cresp(trans->task);
        }
...
void MemoryController::push_pending_TransactionQue(Transaction *trans) {
    ...
    msg.burst_cnt = 0;
    msg.data_ready_cnt = 0;
    msg.data_collected_cnt = 0;
