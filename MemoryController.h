...
public:
    ...
    void ReturnData_statistics(uint64_t task, uint64_t timeAdded, unsigned qos, unsigned mid, unsigned pf_type, unsigned rank, bool wb);
...
//    vector <bool> rank_send_pbr;
    ...
    uint64_t next_ps_cycle;       // 下一次触发巡检的时间点    
    uint64_t current_ps_addr;     // 当前巡检的物理地址
    uint64_t ps_task_counter;
    
    // 为了让 DMC 调度器能识别，需要同时维护巡检的物理坐标
    uint32_t ps_rank;
    uint32_t ps_sid;
    uint32_t ps_group;
    uint32_t ps_bank;
