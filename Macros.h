...
#define PRINTN(str) if (channel_ohot == (channel_ohot & PRINT_CH_OHOT) && now() >= DEBUG_START_TIME && now() <= DEBUG_END_TIME) DDRSim_log <<std::fixed<<str; DDRSim_log.flush();
#define STATE_PRINTN(str) {state_log <<std::fixed<< str; state_log.flush();}
#define TRACE_PRINT(str) if (channel_ohot == (channel_ohot & PRINT_CH_OHOT)) {trace_log<<std::fixed<<str; trace_log.flush();}
