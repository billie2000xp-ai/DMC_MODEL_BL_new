...
//    static uint64_t w_address = 0x80000000;
    ...
    if (SEQ_NUM == 0 && !is_test_cmd) {
        ...
        if (seq_cnt == 0) {
            is_read = (unsigned(rand() % 100) >= WR_RATIO);
            static std::mt19937_64 addr_gen(0);
            uint64_t addr_rand = (addr_gen() & (0xFFFFFFFFFFFFFFFF << align))
...
//        transaction.qos = is_read ? ((rand()%10) ? 3 : 0): 1;
        ...
        bool alt_chan_credit = NUM_CHANS > 1 && OutstandingQueue[transaction.channel ^ 0x00000001].size() < BKD_OSTD;
        if (!chan_credit && alt_chan_credit) {
            transaction.channel = transaction.channel ^ 0x00000001;
            chan_credit = true;
        }
        if (chan_credit) {
