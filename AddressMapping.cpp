...
void addressMapping(Transaction &trans) {
    ...
    uint8_t sid0,sid1,sid2,bg0,bg1,bg2,bg3,bg4,bg5,ba0,ba1,ba2,ba3,ba4,ba5,ba6,ra0,ra1,ra2;
    ...
    uint32_t decoded_row = row0 | (row1<<1) | (row2<<2) | (row3<<3) | (row4<<4) | (row5<<5) | (row6<<6) | (row7<<7) |
    ...
    trans.sid = sid0 | (sid1<<1) | (sid2<<2); // PCH 映射给 trans.sid
    ...
    bg5 = bit_xor(MATRIX_BG5, trans.address);
    // bg6 = bit_xor(MATRIX_BG6, trans.address);
    uint8_t decoded_bg = bg0 | (bg1<<1) | (bg2<<2) | (bg3<<3) | (bg4<<4) | (bg5<<5);
    ...
    // ==========================================================
    // 地址空洞处理 (MAT % 26)
    // 根据 ROW[15:0] = {MAT[4:0], PL[2:0], WL[7:0]} 拆解
    // ==========================================================
    uint8_t decoded_wl  = decoded_row & 0xFF;           
    uint8_t decoded_pl  = (decoded_row >> 8) & 0x07;    
    uint8_t decoded_mat = (decoded_row >> 11) & 0x1F;   

    // 消除 MAT 空洞：将 {BG[3:0], MAT[4:0]} 合并线性化
    // BG 占用 4 bit, MAT 占用 5 bit，故合并时 decoded_bg 需左移 5 位
    uint32_t combined_bg_mat = (decoded_bg << 5) | decoded_mat;

    // 按 26 的倍数拆分，映射到真实的物理结构
    uint8_t real_bg  = combined_bg_mat / 26;
    uint8_t real_mat = combined_bg_mat % 26;

    trans.group = real_bg;
    
    // 重新拼接真实 ROW：{真实MAT[4:0], PL[2:0], WL[7:0]}
    trans.row = (real_mat << 11) | (decoded_pl << 8) | decoded_wl;
    ...
    trans.bankIndex = trans.bank + trans.group * (NUM_BANKS / NUM_SIDS / NUM_GROUPS) +
            trans.sid * (NUM_BANKS / NUM_SIDS) + trans.rank * NUM_BANKS;
}
...
void CalcMatrixNum() {
    ...
    // ====================================================================
    // Geneva V150 ES 容量与数量的硬覆写
    // ====================================================================
    if (IS_GD2) {
        NUM_GROUPS = 40;
        NUM_ROWS = 53248; // 精确的有效行总数
        NUM_BANKS = NUM_BANKS * NUM_GROUPS * NUM_SIDS; // 40 个 Bank
    } else {
